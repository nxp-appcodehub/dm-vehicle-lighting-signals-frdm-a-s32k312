...
void HandleBlinkSignals(void) {
	...
	if (ui32BlinkCounter >= BLINK_CYCLES) {
		...
		if (bLeftBlinkActive == TRUE) {
			if (bBlinkState == TRUE) {
				SetLed(LED_TURN_LEFT_FRONT, COLOR_AMBER_R, COLOR_AMBER_G,
						COLOR_AMBER_B);
				SetLed(LED_TURN_LEFT_REAR, COLOR_AMBER_R, COLOR_AMBER_G,
						COLOR_AMBER_B);
			} else {
		...
		if (bRightBlinkActive == TRUE) {
			if (bBlinkState == TRUE) {
				SetLed(LED_TURN_RIGHT_FRONT, COLOR_AMBER_R, COLOR_AMBER_G,
						COLOR_AMBER_B);
				SetLed(LED_TURN_RIGHT_REAR, COLOR_AMBER_R, COLOR_AMBER_G,
						COLOR_AMBER_B);
			} else {
...
void ToggleLedPattern(uint8 ui8ButtonId) {
	...
	case 1U: /* Low beam toggle - LED 13 and 14 only */
		if (IsLedActive(13U, COLOR_WARM_WHITE_R, COLOR_WARM_WHITE_G,
				COLOR_WARM_WHITE_B) == TRUE) {
			...
			SetLed(13U, COLOR_WARM_WHITE_R, COLOR_WARM_WHITE_G,
					COLOR_WARM_WHITE_B);
			SetLed(14U, COLOR_WARM_WHITE_R, COLOR_WARM_WHITE_G,
					COLOR_WARM_WHITE_B);
			bLowBeam = TRUE;
		}
		break;
	...
	case 2U: /* High beam toggle - LEDs 8, 9, 10, 11 */
		if (bHighBeam == TRUE) {
			...
			if (bLowBeam == TRUE) {
				bHighBeam = TRUE;
				SetLed(LED_HIGH_BEAM_LEFT_1, COLOR_COOL_WHITE_R,
						COLOR_COOL_WHITE_G, COLOR_COOL_WHITE_B);
				SetLed(LED_HIGH_BEAM_LEFT_2, COLOR_COOL_WHITE_R,
						COLOR_COOL_WHITE_G, COLOR_COOL_WHITE_B);
				SetLed(LED_HIGH_BEAM_RIGHT_1, COLOR_COOL_WHITE_R,
						COLOR_COOL_WHITE_G, COLOR_COOL_WHITE_B);
				SetLed(LED_HIGH_BEAM_RIGHT_2, COLOR_COOL_WHITE_R,
						COLOR_COOL_WHITE_G, COLOR_COOL_WHITE_B);
			}
		}
		break;
